...
export const TaskDetails = ({ task, setShowDetails }: TaskDetailsProps) => {
    ...
    return (
        <div
            className="absolute overflow-y-auto top-0 left-0 w-full h-full rounded-lg backdrop-blur-sm p-4 z-50 text-sm"
            style={{
                border: `1px solid var(--${currentTheme}-accent-grey)`, // Use theme color for border
                backgroundColor: `var(--${currentTheme}-task-details-background)`, // Use theme color for background with low opacity
        ...
        >
            <div className="flex flex-row justify-between items-center mb-2">
                <h3
                    ...
                    style={{
                        backgroundColor: `var(--${currentTheme}-background-300)`, // Use theme color for header background
                        color: `var(--${currentTheme}-text-default)`, // Use theme color for header text
                    }}
                >
                    {task.taskName ? (
